...
set "BOOT_LOG=%LOG_DIR%\pitcher_props_cmd_bootstrap.log"

if not exist "%LOG_DIR%" mkdir "%LOG_DIR%"

echo [%DATE% %TIME%] Starting pitcher props CMD wrapper>> "%BOOT_LOG%"
...
"%POWERSHELL_EXE%" -NoProfile -ExecutionPolicy Bypass -File "%PROJECT_DIR%\scripts\run_pitcher_props_task.ps1" -ProjectDir "%PROJECT_DIR%" -PythonExe "%PYTHON_EXE%" -ExportHistory -RunNote "scheduled full pregame run" >> "%BOOT_LOG%" 2>&1
